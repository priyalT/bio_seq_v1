    ...
    def search_single(self, seq_obj: sequence, motif: str, max_mismatches: int):
        ...
        for kmer, pos in self.kmer_generation(seq):
            if self.mismatches(motif, kmer) <= max_mismatches:
                matches.append(Match(seq_obj.id, pos, kmer, None))
        return matches

    def search_fasta(
        self, fasta_sequences: List[sequence], motif: str, mismatches: int
    ):
        all_matches = []
        for seq_obj in fasta_sequences:
            all_matches.extend(self.search_single(seq_obj, motif, mismatches))
        return all_matches
